...
public final class TakeSubcommand {
    ...
    public TakeSubcommand(final EconomyService economy, final CurrencyDefinition definition, final EconomyMessageDispatcher messages, final String permissionRoot) {
    ...
    public CommandDescriptor build(final String namespace) {
        return CommandDescriptor.builder(namespace, "take")
                .permission(PermissionPolicy.of(permissionRoot + ".admin.take"))
                .description("Take currency from a player")
                ...
                .executor(ctx -> {
                    ...
                    if (ctx.argCount() < 2) {
                        return new CommandResult.Usage("/" + ctx.label() + " take <player> <amount>");
                    }
    ...
    private void handleResult(final CommandSender sender, final String targetName, final BigDecimal amount, final TransactionResult result) {
        ...
        if (result instanceof TransactionResult.Success) {
            definition.messages().find("taken-sender").ifPresent(msg -> messages.send(
                    sender,
                    msg,
            ...
            if (online != null) {
                definition.messages().find("taken-receiver").ifPresent(msg -> messages.send(
                        online,
                        msg,
