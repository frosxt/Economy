...
public final class SetSubcommand {
    ...
    public SetSubcommand(final EconomyService economy, final CurrencyDefinition definition, final EconomyMessageDispatcher messages, final String permissionRoot) {
    ...
    public CommandDescriptor build(final String namespace) {
        return CommandDescriptor.builder(namespace, "set")
                .permission(PermissionPolicy.of(permissionRoot + ".admin.set"))
                ...
                .executor(ctx -> {
                    ...
                    if (ctx.argCount() < 2) {
                        return new CommandResult.Usage("/" + ctx.label() + " set <player> <amount>");
                    }
                    ...
                    final TransactionResult result = economy.set(new SetBalanceRequest(
                            target.getUniqueId(),
                            definition.key(),
                            amount,
                            "admin-set"
    ...
    private void handleResult(final CommandSender sender, final String targetName, final BigDecimal amount, final TransactionResult result) {
        ...
        if (result instanceof TransactionResult.Success) {
            definition.messages().find("set-sender").ifPresent(msg -> messages.send(
                    sender,
                    msg,
            ...
            if (online != null) {
                definition.messages().find("set-receiver").ifPresent(msg -> messages.send(
                        online,
                        msg,
        ...
        if (result instanceof final TransactionResult.BelowMin below) {
            definition.messages().find("below-min").ifPresent(msg -> messages.send(
                    sender,
                    msg,
                    Map.of(
                            "player", targetName,
                            "min", BalanceSubcommand.formatAmount(below.minBalance(), definition)
                    )
            ));
            return;
        }
